...
```{r pop_plot_2, echo = FALSE, results = "hide"}
# generate shapefile for the plot
# first create an index
dist_index <- which(dist_select$NAME == table_one$District)
dist_select$mean <- table_one$Mean[dist_index]

# create a bespoke palette
colour_p <- colorRampPalette(brewer.pal(brewer.pal.info["YlGnBu",1], "YlGnBu"))(length(dist_select$mean))
dist_select$colours <- colour_p[as.numeric(cut(dist_select$mean, breaks = (length(dist_select$mean))))]
...
print(plot(dist_select,
           add = TRUE,
           col = dist_select$colours,
           lty = 3))
```
